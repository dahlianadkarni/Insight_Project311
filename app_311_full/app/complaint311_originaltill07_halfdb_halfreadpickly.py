...
def topcomplaint(locationstring,period):
    ...
    with db_test:
        query = "SELECT complaint_type,count(*) AS counts FROM " + dbtable + " WHERE created_date IS NOT NULL AND " +locationstring + " GROUP BY complaint_type ORDER BY counts DESC  LIMIT 10;"
...
def get_zscore(location,period,res,borough_or_cb,printfig):
    ...
    for i in range(TopN):
        ...
        if (borough_or_cb == 'borough') and (period in [1,3]):
...
#         Y_hat_test, sigma = my_linfit(X_train,Y_train, X_test,linregmodel)
    ...
    if printfig:   
        if (borough_or_cb == 'borough') and (period in [1,3]):
            ...
            complaint_here = maxcomplaint2.replace('-','_').replace(' ','_').replace('\n','_').replace('*','').replace('(','').replace(')','').replace('/','_')
            filenametest = "311PKLData/"+borough + "_test" +str(period)+"_"+ complaint_here + ".pkl"
            cv_test = pd.read_pickle(filenametest)
            filenametrain = "311PKLData/"+borough + "_train"+str(period)+"_" + complaint_here + ".pkl"
            cv_train = pd.read_pickle(filenametrain)  
            cv_test = cv_test.sort_index().resample(res,how=sum)
            cv_train = cv_train.sort_index().resample(res,how=sum)
            cv2 = pd.concat([cv_train,cv_test])
    
        else:
            fig_res = res 
            maxcomplaint   
            with db_all:
                query = "SELECT created_date, complaint_type FROM complaints WHERE created_date IS NOT NULL AND community_board = '" + cb + "' AND complaint_type = '" + maxcomplaint + "' ;"
                cv = psql.read_sql(query, parse_dates=['created_date'], con=db_all).set_index('created_date')          
            cv = cv.sort_index().resample(res,how=len) 
     
            maxcomplaint2 = topcomplaints[0]['complaint_type']
            with db_all:
                query = "SELECT created_date, complaint_type FROM complaints WHERE created_date IS NOT NULL AND community_board = '" + cb + "' AND complaint_type = '" + maxcomplaint2 + "' ;"
